(function () {
  ...
  function updateThemeIcon() {
    ...
    /* The sun/moon icons are swapped by CSS based on the data-theme attribute. */
  }
    ...
    for (var i = 0; i < PHASES.length; i++) {
      ...
      for (var j = 0; j < p.lessons.length; j++) {
        ...
        if (hasProgress && p.lessons[j].url) {
          var lp = window.BEProgress.extractPath(p.lessons[j].url);
          if (lp) userDone = window.BEProgress.isLessonComplete(lp);
        }
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      ...
      var userComplete = hasProgress && lessonPath && window.BEProgress.isLessonComplete(lessonPath);
      if (userComplete) userDone++;
      if (l.words) totalWords += l.words;

      // The whole row is the link, so the lesson page is the one destination.
      // Lessons without a readable page fall back to their source on GitHub.
      var readable = (l.status === 'complete' || userComplete) && lessonPath;

      // Second line: what kind of lesson it is, how long it is, and for
      // anything not written yet, where it stands.
      var meta = '<span class="modal-lesson-type" data-type="' + escapeHtml(l.type) + '"' + (l.combines ? ' title="Combines: ' + escapeHtml(l.combines) + '"' : '') + '>' + escapeHtml(l.type) + '</span>';
      if (l.words) {
        meta += '<span class="modal-lesson-dot">·</span>' + formatNum(l.words) + ' words';
        meta += '<span class="modal-lesson-dot">·</span>~' + readMinutes(l.words) + ' min';
      }
      if (l.status !== 'complete') {
        meta += '<span class="modal-lesson-dot">·</span><span class="modal-lesson-state">' + escapeHtml(l.status) + '</span>';
      }

      var body =
        '<span class="modal-lesson-name">' + escapeHtml(l.name) + '</span>' +
        '<span class="modal-lesson-meta">' + meta + '</span>';

      html += '<div class="modal-lesson' + (userComplete ? ' user-done' : '') + '">';
      html += '<span class="modal-lesson-num">' + String(i + 1).padStart(2, '0') + '</span>';
      if (readable) {
        html += '<a class="modal-lesson-main" href="lesson.html?path=' + lessonPath + '">' + body + '</a>';
      } else if (l.url) {
        html += '<a class="modal-lesson-main" href="' + l.url + '" target="_blank" rel="noopener">' + body + '</a>';
      } else {
    ...
    var totalsEl = document.getElementById('modalTotals');
    if (totalsEl) {
      var totals = [p.lessons.length + (p.lessons.length === 1 ? ' chapter' : ' chapters')];
      if (totalWords) {
        totals.push(formatNum(totalWords) + ' words');
        totals.push('~' + formatDuration(totalWords) + ' total');
      }
    ...
    for (var t = 0; t < toggles.length; t++) {
      toggles[t].addEventListener('click', function (e) {
        ...
        if (!path || !window.BEProgress) return;
        if (window.BEProgress.isLessonComplete(path)) {
          window.BEProgress.unmarkLessonComplete(path);
        } else {
          window.BEProgress.markLessonComplete(path);
        }
      });
    }
  ...
  if (window.BEProgress) {
    window.BEProgress.onChange(function () {
  ...
  // 200 wpm, a common estimate for reading to understand, not to skim.
  function readMinutes(words) {
    return Math.max(1, Math.round(words / 200));
  }

  function formatDuration(words) {
