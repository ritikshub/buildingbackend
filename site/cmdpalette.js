 ...
 *   CmdPalette.open()   opens the palette
 *   CmdPalette.close()  closes the palette
 *
...
(function () {
  ...
  // ── Search index ─────────────────────────────────────────────────────
  /**
   * Flatten the inline Markdown that terms.md and lesson docs use (`code`,
   * **bold**, *italic*) to plain text. Results are one-line and already carry
   * <mark> highlighting, so markers are stripped rather than rendered.
   */
  function stripMd(s) {
      ...
      for (var i = 0; i < PHASES.length; i++) {
        ...
        for (var j = 0; j < phase.lessons.length; j++) {
          ...
          _index.push({
            ...
            name:       lesson.name     || '',
            summary:    stripMd(lesson.summary),
    ...
    if (typeof ARTIFACTS !== 'undefined' && Array.isArray(ARTIFACTS)) {
      for (var a = 0; a < ARTIFACTS.length; a++) {
        ...
        _index.push({
          ...
          name:       art.name || '',
          summary:    stripMd(art.description),
  ...
  function open() {
    if (_isOpen) {
      // Already open, so just make sure the input is focused
      var inp = _inputEl();
      if (inp) inp.focus();
      return;
    }
  ...
  document.addEventListener('keydown', function (e) {
    if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
      e.preventDefault();
      if (_isOpen) {
        // Palette is already open, so just refocus the input
        var inp = _inputEl();
        if (inp) inp.focus();
      } else {
        open();
