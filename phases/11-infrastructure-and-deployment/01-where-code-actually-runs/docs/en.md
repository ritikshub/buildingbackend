...
docker compose exec -T app python \
  phases/11-infrastructure-and-deployment/01-where-code-actually-runs/code/compute_ladder.py
```
...
**`nr_throttled` is the container metric nobody graphs.** Non-zero means the kernel stopped your runnable threads to enforce `cpu.max`. Put it on a dashboard next to your latency percentiles ([Metrics: Counters, Gauges & Histograms from Scratch](../../10-logging-monitoring-and-observability/05-metrics-from-scratch/) covers making it a counter you can alert on) and a whole class of unexplained tail latency becomes a five-second diagnosis.
...
- **Graph `container_cpu_cfs_throttled_periods_total` / `nr_throttled` next to your p99.** Unexplained tail latency in a CPU-limited container is throttling until proven otherwise. See [Health Checks & Probes](../../10-logging-monitoring-and-observability/08-health-checks-and-probes/) for the related failure where a throttled process fails its own liveness probe and gets restarted for being throttled.
