...
**A secret copied into a layer is in that layer forever.** The measured build proves both halves at once: `/tmp/deploy_key` is **absent from the merged filesystem** — so a shell inside the running container cannot find it, and a scanner that inspects the running container reports nothing — and it is **still in layer 2 (`9d74698ed8d1`), 84 bytes**, from which the script reads it straight back out: `'AAAAdeploy-key-do-not-ship-9f31c0'`. Anyone who can pull the image can do the same with `tar`. There is no fix after the fact; there is only rotation, because **publishing the image was the disclosure.** ([Secrets Management & Rotation](../../08-auth-and-security/13-secrets-management-and-rotation/) covers the rotation side.)
...
docker compose exec -T app python \
  phases/11-infrastructure-and-deployment/03-images-layers-and-builds/code/image_builder.py
```
