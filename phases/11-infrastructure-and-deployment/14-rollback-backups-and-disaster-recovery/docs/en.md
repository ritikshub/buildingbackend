...
- **A consumed message.** Once a consumer has advanced its offset past a message and the topic's retention window has moved on, the old consumer cannot re-read it. Dropping support for an old payload version is the same class of change — see [Schema Evolution & Event Contracts](../../07-messaging-and-pub-sub/12-schema-evolution-and-event-contracts/).
...
- **An offline or logically air-gapped copy** for the longest retention tier, with restore credentials that are not stored in the same secret manager as everything else — see [Secrets Management & Rotation](../../08-auth-and-security/13-secrets-management-and-rotation/).
...
docker compose exec -T app python \
  phases/11-infrastructure-and-deployment/14-rollback-backups-and-disaster-recovery/code/rollback_and_recovery.py
```
...
**Section 4** is point-in-time recovery against a write-ahead log, and it is Phase 4's WAL used for the purpose that pays for itself:
