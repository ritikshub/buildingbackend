...
3. **Caching that is always correct.** A content address can never go stale, so any layer you have already downloaded is valid forever. (The same reasoning underlies a strong `ETag`; see [HTTP Caching & ETags](../../06-caching/08-http-caching-and-etags/).)
...
docker compose exec -T app python \
  phases/11-infrastructure-and-deployment/04-registries-and-supply-chain/code/registry_and_trust.py
```
