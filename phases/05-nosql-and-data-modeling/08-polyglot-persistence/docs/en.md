...
The naïve reading of Phase 5 is "use the best database for each job," and if you follow it literally
...
| Slice of data | Pressure (Lesson 1) | Store | Phase 5 lesson |
|---|---|---|---|
| Orders, payments | none — needs ACID | Relational (Postgres) | — (Phase 4) |
...
(Phase 4, Lesson 7) — lifted from columns-within-a-database up to databases-within-a-system.
...
**Change Data Capture (CDC).** The elegant generalization — and a beautiful callback to Phase 4. Every
relational database already writes a **write-ahead log (WAL)**: an ordered, durable record of every
committed change (Phase 4, Lesson 13 — the log you *built*). **CDC tails that log** (tools like
...
**at-least-once** — a crash mid-publish means an event may be delivered twice (Phase 7; glossary:
...
make the update a set-to-this-value rather than an increment — the idempotency discipline from Phase 3,
...
3. Change Data Capture uses which structure from Phase 4 as its backbone, and why is it elegant that
...
- Sync stores **without** a distributed transaction: the **outbox pattern** (write data + event in one
  atomic DB transaction, relay publishes) and **CDC** (tail the **WAL** from Phase 4 and stream every
...
Next: [Why & Where to Cache](../../06-caching/01-why-and-where-to-cache/) — you've just seen the cache
appear as one of the derived stores in a polyglot system; Phase 6 goes deep on it, starting with the
