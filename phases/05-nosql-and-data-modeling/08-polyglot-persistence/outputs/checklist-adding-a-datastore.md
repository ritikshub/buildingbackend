...
The whole of Phase 5 in one gate. Run it *before* introducing any new database, cache, index, or
...
- [ ] Rule out the cheap relational fixes first: a missing **index**, an **N+1** query, no
      **connection pool**, no **caching** (Phase 4). Most "we need a new database" is a tuning bug.
