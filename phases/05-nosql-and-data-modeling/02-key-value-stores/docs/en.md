...
An in-memory dictionary forgets on restart (Phase 4, Lesson 1). To make a key-value store
...
(Phase 4, Lessons 8–9). In-place updates mean random disk writes, and every write maintains the
...
- **Crash recovery is trivial** — on startup, replay the log start-to-end, and the *last* record
  ...
  everything before it is intact (this is the write-ahead-log idea from Phase 4, Lesson 13, taken
...
r.set("session:9f3a", "...", ex=3600)                 # with a 3600s TTL — auto-expire (Phase 6)
...
which is why people call it a "data structure server." You'll go deep on Redis in Phase 6.
