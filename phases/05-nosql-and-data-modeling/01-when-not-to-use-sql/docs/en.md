...
You spent all of Phase 4 learning why the relational database is so good: it stores data as
...
- **A rich, declarative query language.** You describe *what* you want (SQL) and the planner
  figures out *how* (Phase 4, Lesson 10).
...
- **Rapidly-evolving data**, where the fields change weekly and every change is an `ALTER TABLE`
  migration (Phase 4, Lesson 15) coordinated across a running system.
...
A classic RDBMS funnels every write through **one primary node** (Phase 4's WAL and MVCC assume
...
exactly that pressure.** The rest of Phase 5 teaches those stores so that when the moment comes,
...
1. A teammate says "our app is slow, we should switch to NoSQL." Which of the five pressures
   ...
   likely relational fix (from Phase 4) they may have skipped?
