...
and every property that made Postgres great in Phase 4 now works against you:

- **The index is the wrong shape and it never stops growing.** A B-tree index on `(host, metric,
  ts)` has to be maintained on every one of those million inserts per second (Phase 4, Lesson 9),
...
- **You never update a point, but you pay for the machinery that lets you.** MVCC, row versions,
  vacuum (Phase 4, Lesson 12) all exist to make in-place updates safe. Time-series data is
