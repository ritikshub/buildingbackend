...
This is the HTTP request/response you built in Phase 2, and it is the correct model whenever the caller **cannot make progress without the answer**:
...
**You gain eventual consistency.** The system is deliberately, briefly wrong: the order exists before the email is sent, before the warehouse knows, before analytics updates. Different parts of the system disagree for seconds or minutes. It converges — *eventually* — but any code that assumes "if the order row exists, the confirmation was sent" is now a bug. This is the same ACID-versus-BASE trade from Phase 5, resurfacing at the messaging layer. The hard part is rarely technical; it is that a product manager must accept a UI that says "processing" instead of "done".
