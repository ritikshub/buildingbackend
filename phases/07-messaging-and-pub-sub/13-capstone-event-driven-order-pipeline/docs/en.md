...
- **Tracing across every hop.** The envelope carries `traceparent`; nothing here consumes it. Real value requires a span per hop, linked across the async boundary, so "why did this customer not get their email?" is a query rather than an investigation ([Phase 10](../../10-logging-monitoring-and-observability/)).
...
Next: [Authentication, Authorization & the Security Mindset](../../08-auth-and-security/01-authn-authz-and-the-security-mindset/) — this pipeline moves money and personal data between services that never authenticate each other, over a broker that will hand any message to anyone who subscribes. Phase 8 is about who is allowed to do what, and how you prove it.
