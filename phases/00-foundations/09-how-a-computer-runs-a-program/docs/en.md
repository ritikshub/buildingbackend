...
- **Stack**: a scratchpad that grows with every function call and shrinks when it
  returns (local variables, where to return to). Infinite recursion overflows it,
  giving a **stack overflow**.

### The Operating System: the manager

You run many programs "at once" (a browser, an editor, your server) but there are only
a handful of CPU cores. The **operating system (OS)**, whether Linux, macOS or Windows,
is the manager that makes this work:
...
- **Mediates hardware.** A program can't touch the disk or network directly; it asks the OS
  ...
  mode" and the OS in privileged "kernel mode," is why one crashing program doesn't take
  down the computer.)
...
- A **process** has a PID and its own protected memory, laid out as **code / data / heap /
  stack**, which is where memory leaks and stack overflows happen.
