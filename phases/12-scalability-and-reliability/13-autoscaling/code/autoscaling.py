#!/usr/bin/env python3
"""
Phase 12, Lesson 13 - Autoscaling: Control Loops That Don't Oscillate.
...
# The SLO is derived, not asserted. Phase 9 L11 measured W = S/(1-rho) to within
...
def simulate(cfg: Cfg, noise: List[float]) -> Res:
    ...
    while t < cfg.horizon:
        ...
        # A queue older than the caller's deadline is abandoned (Phase 9 L11).
...
def section3(noise: List[float]) -> None:
    ...
    print("  L = lambda x W (Phase 9 L01): it is proportional to load in BOTH regimes.")
...
def metastable_run(mode: str) -> Tuple[List[Tuple[float, float, int, int, float,
    ...
    while t < horizon:
        ...
        if mode == "shed":
            # An adaptive concurrency limit (Phase 9 L11): admit only what the
...
def section5() -> None:
    ...
    print("  its throughput follows the Universal Scalability Law (Phase 12 L02):")
    ...
    print("  the exit is load shedding - Phase 9 L11 built it.")
    print()
