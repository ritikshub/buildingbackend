...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 880 468" width="100%" style="max-width:840px" role="img" aria-label="Measured fleet size over a 70 minute run for two identical target-tracking autoscalers that differ only in dead time, plotted against the dashed demand curve that shows how many instances are actually needed. With zero dead time the green curve tracks demand exactly and settles at 24 instances on the flat plateau. With 210 seconds of dead time the red curve overshoots to 145 instances and collapses to 6, repeatedly, while the traffic underneath it does not change.">
  <g font-family="'JetBrains Mono', ui-monospace, monospace">
    ...
    <text x="112" y="82" font-size="10" fill="#c94a12" font-weight="700">- -  demand / 100 req-per-instance = the fleet you need</text>
...
- **In-flight requests.** Terminating an instance kills the requests it is serving unless you drain first. You need connection draining at the load balancer and graceful shutdown in the process — deregister, stop accepting, finish what you have, then exit. [Health Checks & Probes](../../10-logging-monitoring-and-observability/08-health-checks-and-probes/) builds this; the autoscaling-specific part is that the *termination hook* must be longer than your longest request, and it usually is not.
...
docker compose exec -T app python \
  phases/12-scalability-and-reliability/13-autoscaling/code/autoscaling.py
```
...
The SLO is **derived, not asserted.** Phase 9's lesson measured `W = S/(1−ρ)` to within 3% of theory, so at ρ = 0.90 the response time is exactly ten times the service time. A tick in which an instance runs above ρ = 0.90 is a tick in which its requests miss the 120 ms objective, and that is what the `SLO%` column counts. Capacity per instance is `min(cpu_capacity, slots / service_time)` — **which resource binds depends on the service time**, and that single line is what makes section 3 work.
...
== 5 · AUTOSCALING INTO A METASTABLE FAILURE ==
  ...
  its throughput follows the Universal Scalability Law (Phase 12 L02):
...
`lagThreshold` is a per-replica target, exactly like an HPA `averageValue`. [Phase 7's lesson on consumer lag](../../07-messaging-and-pub-sub/09-backpressure-lag-and-flow-control/) explains why lag is the right signal and what it does not tell you. Two traps specific to autoscaling here: **you cannot have more useful consumers than partitions**, so `maxReplicaCount` above the partition count buys idle pods; and scaling a consumer group triggers a **rebalance**, which pauses consumption, which raises lag, which can make the autoscaler scale again. Cooldowns matter more here than anywhere else.
...
For the metrics pipeline behind all of this — scrape intervals, aggregation windows, what a gauge can miss — see [Phase 10](../../10-logging-monitoring-and-observability/). For the deployment mechanics that decide how fast an instance can boot, see [Phase 11](../../11-infrastructure-and-deployment/).
