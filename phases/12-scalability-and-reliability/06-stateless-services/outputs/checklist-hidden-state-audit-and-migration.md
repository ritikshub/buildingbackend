...
(Phase 6), rate-limit algorithms (Phase 3 L09), or deployment mechanics (Phase 11).
...
- [ ] **Phase 2 — Dual-write.** Write to memory *and* the store; keep reading from memory
      only, so request handling cannot break.
      **Gate:** `session_store_writes_total` rate ≈ login rate. Hold ≥ 24 h.
- [ ] **Phase 3 — Read store first, fall back to memory, repair on miss.** Emit `store_fallback_total`.
...
- [ ] **Phase 4 — Remove the in-memory read, then the write.** Then disable sticky sessions.
...
- [ ] **Phase 5 — Remove affinity config entirely, then allow scale-in.** Only now.
...
**The common failure:** disabling stickiness during Phase 3 because "the store works now", then discovering the fallback path carried more traffic than anyone thought.
