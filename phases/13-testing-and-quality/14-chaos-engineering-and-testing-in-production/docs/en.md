...
| **dependency loss** | a whole service, a whole zone | [Failure Domains, Blast Radius & Shuffle Sharding](../../12-scalability-and-reliability/09-failure-domains-and-shuffle-sharding/) |
...
Testing in production has a legitimate, boring, everyday form that predates the word chaos: **a canary** — release to a small fraction of traffic, compare it against the baseline, promote or roll back. [Deployment Strategies: Rolling, Blue-Green & Canary](../../11-infrastructure-and-deployment/11-deployment-strategies/) covers the mechanics and [Deploy ≠ Release: Feature Flags & Progressive Delivery](../../11-infrastructure-and-deployment/12-deploy-vs-release-feature-flags/) covers the control plane. What almost nobody does is notice that the comparison step is a **hypothesis test**, and that writing it as a threshold does not avoid the statistics — it just does them badly.
...
```bash
python3 phases/13-testing-and-quality/14-chaos-engineering-and-testing-in-production/code/chaos.py
```
