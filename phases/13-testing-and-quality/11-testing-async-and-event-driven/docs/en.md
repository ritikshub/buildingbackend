...
- **The transactional outbox as a test seam.** Phase 7's [Dual Write, the Outbox Pattern & CDC](../../07-messaging-and-pub-sub/10-dual-write-outbox-and-cdc/) exists to make the write and the publish atomic. It also gives the test a table it can read: *the effect and its notification are one row*, so "has this been processed?" becomes a `SELECT`.
...
One detail that is load-bearing and easy to get wrong: the key must be derived from the **business event**, not from the transmission. A key generated by the producer at send time gives every redelivery a fresh key and defeats every dedup mechanism downstream — the trap Phase 7 spells out in full.
...
A consumer that throws gets retried, backs off, and eventually lands in a dead-letter queue. Phase 7's [Retries, Backoff & Dead-Letter Queues](../../07-messaging-and-pub-sub/08-retries-backoff-and-dead-letter-queues/) builds that path; this is how you assert on it.
...
```bash
python3 phases/13-testing-and-quality/11-testing-async-and-event-driven/code/async_testing.py
```
