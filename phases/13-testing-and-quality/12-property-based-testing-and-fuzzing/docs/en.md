...
- **Idempotence.** `f(f(x)) == f(x)`. Normalisers, upserts, retried handlers ([Idempotency & Safe Retries](../../03-api-design/07-idempotency-safe-retries/) is the whole lesson).
...
```bash
python3 phases/13-testing-and-quality/12-property-based-testing-and-fuzzing/code/property_testing.py
```
