...
USE comes **before** profiling, not after, because it decides which profiler you need. Saturated CPU → on-CPU profiler. Saturated pool or high off-CPU time → wall-clock profiler, or just fix the pool. Growing memory → memory profiler. The [RED and USE dashboards lesson](../../10-logging-monitoring-and-observability/11-dashboards-red-and-use/) builds exactly these panels.
