...
- **Always bound a fan-out with a `Semaphore`.** `gather` over 10,000 URLs creates 10,000 Tasks and opens as many sockets as your connection limits allow, instantly. That is a load problem, not a concurrency problem: you will exhaust file descriptors, blow through the connection pool ([Phase 4, Lesson 14](../../04-relational-databases/14-connection-pooling-and-n-plus-1/)), and DDoS the service you depend on. A `Semaphore(20)` costs one line. Lesson 11 covers the general form of this — backpressure — and so does [Phase 7, Lesson 09](../../07-messaging-and-pub-sub/09-backpressure-lag-and-flow-control/).
