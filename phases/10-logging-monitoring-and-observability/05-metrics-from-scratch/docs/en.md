...
Name plus labels identifies one **time series** — exactly the measurement-plus-tags model you built a database for in Phase 5, Lesson 5. The value is whatever that series was at each scrape.
...
You met this in Phase 5, Lesson 5, and it is the single most common way engineers destroy a metrics backend. It deserves restating as arithmetic, because that's all it is: **the number of series a metric creates is the product of the number of distinct values each label can take.**
...
Nothing warns you. The code compiles, the tests pass, and the first deploy quietly begins creating one time series per user, each with its own index entry and in-memory chunk in the TSDB you built in Phase 5. The rule is a one-liner, and it is the same rule as for tags in a time-series database:
...
- **Cardinality is a product**: 10 buckets × 20 routes × 5 statuses × 3 regions = 3,000 series (fine); add `user_id` and it's 150 million (dead database — Phase 5, Lesson 5). **Labels are dimensions you group by; identity belongs in logs and traces.**
