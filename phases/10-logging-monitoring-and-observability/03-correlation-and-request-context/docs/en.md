...
checkout. Phase 2's HTTP (HyperText Transfer Protocol) request describes the *client*; nothing in it
...
different value per concurrent request whether the concurrency is threads or coroutines — Phase 9's
...
connection's first request (Phase 2, Lesson 11). Keep it to a few small, low-risk fields — tenant,
...
Whatever the broker, the Phase 7 discipline holds: metadata in headers, payload in the body, and the
...
5. You are asked to put `user_email` in baggage so every downstream service can log it. Give three
   separate reasons to refuse — one about cost, one about security, and one from Phase 5 Lesson 5
